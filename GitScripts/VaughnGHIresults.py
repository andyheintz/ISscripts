...
file_path = "/Users/andyheintz/Desktop/ISenergy/data/mergedVaughnSolar.csv"
df = pd.read_csv(file_path, header=1)
df = df.loc[:, ~df.columns.str.contains('Unnamed')]  # Drop unnamed columns
print(df.columns)

df['Timestamp'] = pd.to_datetime(df[['Year', 'Month', 'Day', 'Hour', 'Minute']], errors='coerce')
df = df.drop(columns=['Year', 'Day', 'Hour', 'Minute'])  # Keep 'Month'
...
T_year, T_day, K_high = 8760, 24, 15  # Keep K=5 for now
...
plt.title('Prediction vs. Actual GHI in Vaughn, NM')
plt.legend()
plt.grid(True)
plt.show()
...
# Monthly meteorological feature mapping
...
results_folder = "/Users/andyheintz/Desktop/ISenergy/results/VaughnGHIResults"
...
plt.xlabel('Date')
plt.ylabel('GHI (W/m²)')
plt.title('Predicted GHI for 2021 in Vaughn, NM')
plt.grid(True)
plt.show()
...
plt.title('Monthly Average Predicted GHI for 2021 in Vaughn, NM')
...
# Apply power output equation
# Final output in kWh per hour since GHI is hourly
...
plt.title("Total Monthly Predicted Solar Energy Output in 2021 for Vaughn, NM")
plt.xticks(rotation=45)
plt.grid(axis='y')
plt.show()
...
panel_capacity_kW = (panel_area_m2 * irradiance_stc * panel_efficiency) / 1000 
