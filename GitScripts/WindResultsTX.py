...
file_path = "/Users/andyheintz/Desktop/ISenergy/data/TexWindSpeed100.csv"
...
numeric_cols = ['WindSpeed100m', 'air temperature at 100m (C)', 'air pressure at 100m (Pa)', 'vertical windspeed at 80m (m/s)']
...
X['VerticalWindSpeed120m'] = df['vertical windspeed at 80m (m/s)']
y = df['WindSpeed100m']
...
future_df['VerticalWindSpeed120m'] = df['vertical windspeed at 80m (m/s)'].mean()
...
results_folder = "/Users/andyheintz/Desktop/ISenergy/results/TexWindResults"
os.makedirs(results_folder, exist_ok=True)
future_predictions_file = os.path.join(results_folder, "WindPredsTX.csv")
...
plt.title("Predicted Wind Speeds for 2021 in Fort Stockton, TX")
plt.legend()
plt.grid(True)
plt.show()
...
plt.title("Total Monthly Predicted Power Output in 2021 for Fort Stockton, TX")
...
feature_importance[:10].plot(kind='barh', color='c', alpha=0.7)  # Show top 10 features
plt.xlabel('Importance Score')
plt.ylabel('Feature Name')
plt.title('10 Most Important Features in Random Forest Model for Wind Speeds in Fort Stockton, TX')
plt.gca().invert_yaxis()  # Flip order so most important feature is at the top
plt.show()
...
total_energy_MWh = total_energy_kWh / 1000  # Convert kWh to MWh
...
plt.title('Prediction vs. Actual Wind Speed in Fort Stockton, TX')
plt.grid(True)
plt.show()
...
first_feature = X_test.columns[0]  # Select first predictor variable
